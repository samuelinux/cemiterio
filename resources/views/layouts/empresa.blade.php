    ...
    <div class="flex h-screen bg-gray-100" x-data="{ sidebarOpen: false }">
        ...
        <div class="fixed inset-y-0 left-0 z-50 w-64 bg-blue-800 transform transition-transform duration-300 ease-in-out lg:translate-x-0 lg:static lg:inset-0"
            ...
            <nav class="mt-5 px-2">
                ...
                <a href="{{ route('empresa.causas-morte.index', auth()->user()->empresa->slug) }}"
                    class="group flex items-center px-2 py-2 text-base font-medium rounded-md text-blue-100 hover:bg-blue-700 hover:text-white {{ request()->routeIs('empresa.causas-morte.*') ? 'bg-blue-900 text-white' : '' }}">
                    <svg class="mr-4 h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div class="absolute bottom-0 w-full p-2">
                <form method="POST" action="{{ route('empresa.logout', auth()->user()->empresa->slug) }}">
                    @csrf
                    <button type="submit"
                        class="w-full group flex items-center px-2 py-2 text-base font-medium rounded-md text-blue-100 hover:bg-blue-700 hover:text-white">
                        <svg class="mr-4 h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <!-- Overlay for mobile -->
        <div class="fixed inset-0 z-40 bg-gray-600 bg-opacity-75 lg:hidden" x-show="sidebarOpen" x-cloak
